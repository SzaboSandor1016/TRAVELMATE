...
class FirebaseRemoteDataSourceImpl: FirebaseRemoteDataSource {
     ...
     */
    override suspend fun uploadTrip(trip: Trip, tripFirebaseIdentifier: TripIdentifier) {

        withContext(Dispatchers.IO) {

            val success = async { uploadIdentifierForTrip(
            ...
            when(success.await()) {
                ...
                false -> {
                    Log.e("FirebaseDatabase", "creating trip data branch failed due to identifier creation error")
                }
     ...
     */
    suspend fun uploadIdentifierForTrip(tripFirebaseIdentifier: TripIdentifier): Boolean
    = suspendCancellableCoroutine {continuation ->
     ...
     */
    override suspend fun fetchMyTrips(uid: String): List<TripIdentifier?> =
        suspendCancellableCoroutine { continuation ->
     ...
     * gets all trips' [TripIdentifier] that the currently signed in user has contributed to
     */
